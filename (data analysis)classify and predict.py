...
test = feature_vectors('test_features.txt')
...
##base_predictions = classifier.predict(test_feats)
...
##falsepos = 0
##falseneg = 0
##for i in range(0, len(base_predictions)):
##    if base_predictions[i] != test_labels[i]:
##        errors += 1
##        if base_predictions[i] == 1:
##            falsepos += 1
##        if base_predictions[i] == 0:
##            falseneg += 1
##print str(errors) + " errors"
##print str(falsepos) + " false positives"
##print str(falseneg) + " false negatives"
...
validate_a = feature_vectors('test_anchor_features.txt')
...
##    print nums[i]
a_rforest = RandomForestClassifier(n_estimators=110, min_samples_leaf=10, max_depth=20)#nums[i])
a_rforest.fit(train_a_feats, train_a_labels)
predictions = a_rforest.predict(validate_a_feats)
...
for i in range(0, len(predictions)):
    if predictions[i] != validate_a_labels[i]:
        errors += 1
        if predictions[i] == 1:
            falsepos += 1
        if predictions[i] == 0:
...
print str(falsepos) + " false positives"
print str(falseneg) + " false negatives"
print str(truepos) + " true positives"
print str(trueneg) + " true negatives"
