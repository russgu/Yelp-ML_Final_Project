...
class Finding_Restaurants:
    
    def finding_restaurants(self, path, business_type, city, debug_flag=False):
        start = time.time()
        res_b_id = [] # qualified businesses (represented by their business id)
        outfile = '(yelp)selected_reviews.json'


        # finding all qualified restaurants
        with open(path+'yelp_dataset_challenge_academic_dataset/yelp_academic_dataset_business.json','r') as json_file:
        ...
        with open(path+'yelp_dataset_challenge_academic_dataset/yelp_academic_dataset_review.json','r') as json_file,\
